// app/(protected)/alliance/[id]/dsb/attendance/page.tsx
'use client'
import { useState, useEffect } from 'react'
import { useParams, useRouter } from 'next/navigation'

interface RosterMember { commander_uid: string; name: string; task_force: string; roster_role: string }
interface AttendanceRecord { commander_uid: string; status: string; remark: string | null }

const STATUSES = ['attended','no_show','late','backup','emergency','afk'] as const
const STATUS_LABEL: Record<string, string> = {
  attended: '✓ Attended', no_show: '✗ No Show', late: '⏱ Late',
  backup: '↑ Backup', emergency: '⚡ Emergency', afk: '💤 AFK',
}
const STATUS_COLOR: Record<string, string> = {
  attended: 'chip-participated', no_show: 'chip-absent',
  late: 'chip-below', backup: 'chip-unselected',
  emergency: 'chip-unselected', afk: 'chip-below',
}

export default function DSBAttendancePage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const res  = await fetch(`/api/dsb/attendance?alliance_id=${allianceId}`)
  ...
  const handleRecord = async (commanderUid: string, status: string, remark = '') => {
    if (!eventId) return
    setSaving(commanderUid); setMsg('')
    try {
      const res = await fetch('/api/dsb/attendance', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ event_id: eventId, commander_uid: commanderUid, status, remark, task_force: activeTab }),
      })
      const data = await res.json()
      if (!res.ok) { setMsg(data.error); return }
      setAttendance(prev => ({ ...prev, [commanderUid]: { commander_uid: commanderUid, status, remark } }))
    ...
    <div className="flex flex-col gap-5 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div className="page-header mb-0">
          <h1 className="page-title">DSB Attendance</h1>
      ...
      {!eventId ? (
        <div className="glass-card p-8 text-center">
          <p className="text-tactical-500">No DSB event found for this week</p>
        </div>
      ) : (
        <>
          {/* TF tabs */}
          ...
          <div className="glass-card p-5">
            <p className="font-semibold text-tactical-900 mb-4">
              Starters — TF-{activeTab} ({starters.length})
            </p>
                  ...
                  <div key={m.commander_uid}
                    ...
                    <div className="flex gap-1 flex-wrap justify-end">
                      {STATUSES.map(s => (
                        <button
                          key={s}
                          onClick={() => handleRecord(m.commander_uid, s)}
                          disabled={saving === m.commander_uid}
                          className={`chip text-xs ${
                            rec?.status === s ? STATUS_COLOR[s] : 'chip-unselected'
                          }`}
                        >
                          {s === 'attended' ? '✓' :
                           s === 'no_show'  ? '✗' :
                           ...
                           s === 'afk'      ? '💤' :
                           s === 'backup'   ? '↑' : '⚡'}
                          <span className="hidden lg:inline ml-1 capitalize">{s.replace('_',' ')}</span>
                        </button>
                      ))}
                    </div>
                  </div>
          ...
          {subs.length > 0 && (
            <div className="glass-card p-5">
              <p className="font-semibold text-tactical-900 mb-4">
                Substitutes — TF-{activeTab} ({subs.length})
              </p>
              <div className="flex flex-col gap-3">
                {subs.map(m => {
                  const rec = attendance[m.commander_uid]
                  return (
                    <div key={m.commander_uid}
                         className="flex items-center justify-between gap-3 p-3 rounded-xl bg-surface-overlay">
                      <p className="text-sm font-medium text-tactical-900 truncate flex-1">{m.name}</p>
                      <div className="flex gap-1 flex-wrap justify-end">
                        {STATUSES.map(s => (
                          <button
                            key={s}
                          ...
                          >
                            {s === 'attended' ? '✓' : s === 'no_show' ? '✗' : s.slice(0,3)}
                          </button>
                        ))}
                      </div>
                    </div>
