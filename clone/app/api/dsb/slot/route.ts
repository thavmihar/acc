...
export async function POST(req: Request) {
  try {
    ...
    if (task_force === 'A' && event.tfa_finalized) {
      return NextResponse.json({ error: 'TF-A is finalized' }, { status: 409 })
    }
    if (task_force === 'B' && event.tfb_finalized) {
      return NextResponse.json({ error: 'TF-B is finalized' }, { status: 409 })
    }

    const field = task_force === 'A' ? 'tfa_slot' : 'tfb_slot'
    const { error } = await supabase
      .from('dsb_events').update({ [field]: slot }).eq('id', event_id)

    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('[DSB SLOT]', err)
