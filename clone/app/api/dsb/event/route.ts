...
export async function GET(req: Request) {
  try {
    ...
    const [{ data: event }, { data: members }] = await Promise.all([
      supabase.from('dsb_events').select('*').eq('alliance_id', allianceId).eq('week_key', weekKey).single(),
    ...
    // If no event exists and user is R4+ create it
    ...
    console.error('[DSB EVENT GET]', err)
