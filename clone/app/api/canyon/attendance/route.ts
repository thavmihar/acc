// app/api/canyon/attendance/route.ts
...
export async function GET(req: Request) {
  try {
    ...
    const { data: event } = await supabase
      .from('canyon_events').select('id')
      .eq('alliance_id', allianceId).eq('week_key', getWeekKey()).single()
    ...
    const [{ data: rosterRaw }, { data: attendanceRaw }] = await Promise.all([
      supabase.from('event_roster').select('commander_uid, task_force, roster_role')
        .eq('event_id', event.id).eq('event_type', 'canyon'),
      supabase.from('attendance_records').select('commander_uid, status, remark')
        .eq('event_id', event.id).eq('event_type', 'canyon'),
    ])

    const uids = (rosterRaw ?? []).map(r => r.commander_uid)
    const { data: commanders } = await supabase.from('commanders').select('uid, name').in('uid', uids)

    const nameMap: Record<string, string> = {}
    for (const c of (commanders ?? [])) nameMap[c.uid] = c.name

    const roster = (rosterRaw ?? []).map(r => ({ ...r, name: nameMap[r.commander_uid] ?? r.commander_uid }))

    return NextResponse.json({ event_id: event.id, roster, attendance: attendanceRaw ?? [] })
  } catch (err) {
    console.error('[CANYON ATTENDANCE GET]', err)
...
export async function POST(req: Request) {
  try {
    ...
    await writeAuditLog({
      action: 'canyon_attendance_recorded', performed_by: auth.commander_uid,
    ...
    console.error('[CANYON ATTENDANCE POST]', err)
